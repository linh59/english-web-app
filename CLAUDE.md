...
- **Tự động nén audio phía client trước khi upload** (`src/shared/lib/audio-compression.ts`,
  ...
  bitrate/sample-rate tự tính theo độ dài file (budget ~45MB, clamp 24-64kbps) để vừa
  giới hạn Storage 50MB. `MAX_UPLOAD_SIZE_BYTES` giờ áp cho file đã nén (không phải file
  gốc); thêm `MAX_RAW_UPLOAD_SIZE_BYTES` (1GB) chặn file gốc quá khổ trước khi thử nén.
  `LessonCard` hiện tiến độ xử lý chunk (`processing_step` — dữ liệu đã có sẵn từ trước
  nhưng chưa từng hiển thị). Đã verify thật: file 436MB/~5h03m nén xong trong 78s không
  crash tab, đúng như tính toán báo lỗi "vẫn quá lớn sau khi nén" (giới hạn V1 đã biết
  cho file cực dài, không phải bug); file ngắn test full pipeline (nén → upload → Gemini
  → lưu câu) transcript + timestamp chính xác, Content-Type lưu đúng `audio/mp4`.
...
4. File cực dài (~5h+) vẫn có thể vượt giới hạn 50MB dù đã nén ở mức bitrate thấp nhất
   (24kbps) — hiện chỉ báo lỗi rõ ràng yêu cầu người dùng chia nhỏ file, chưa có giải
   pháp tự động (vd tăng giới hạn Storage nếu plan cho phép, hoặc chia nhiều file).
...
- **Bitrate/sample-rate tự thích ứng theo độ dài file** (không dùng 1 mức cố định):
  app này target audio "dài >1h" (xem Phạm vi V1), một mức bitrate cố định (vd 48kbps)
  sẽ khiến bài học dài thường xuyên vượt 50MB. Công thức: budget 45MB, `bitrate =
  clamp(budget*8/duration/1000, 24, 64)` kbps, sample rate 16kHz nếu ≤32kbps else
  22.05kHz. Test thật với file 18198s (~5h) cho kết quả nén xong vẫn ~52MB (vượt cap) —
  chấp nhận là giới hạn V1 đã biết cho file cực dài, không cố nén thêm bằng codec khác.
